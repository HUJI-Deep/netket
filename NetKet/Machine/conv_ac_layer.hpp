...
namespace netket {
    ...
    class ConvACLayer : public AbstractLayer<T> {
        ...
        void create_tensors(){
            ...
            interlaced_top_buff_len_ = number_of_output_channels_ * output_height_ * output_width_ * num_regions_;
            interlaced_top_buff_.reset(new Matrix<T, 2, 1>[interlaced_top_buff_len_]);
        }
    ...
    public:
        ...
        ConvACLayer(const json &pars, int number_of_input_channels, int input_height, int input_width) :
                number_of_input_channels_(number_of_input_channels), input_height_(input_height),
        ...
        Eigen::array<int , 3> Noutput() const override {
            return Eigen::array<int , 3>{number_of_output_channels_, output_height_, output_width_};
        }
        ...
        void DerLog(const TensorType &input_tensor, TensorType &next_layer_gradient, VectorType &plat_offsets_grad) {
            Tensor<T, 2> src = Eigen::TensorMap<Tensor<T, 2>>(next_layer_gradient.data(), 1,
                                                              ...
                                                              output_width_);
            Tensor<T, 2> dst = TensorMap<Tensor<T, 2>>(top_expanded_diff_.data(), num_regions_,
            ...
            ggemm_2ops_cpu
                    <T, T, T, uint8_t,
                            ...
                            true, true, true>
                    (number_of_output_channels_, output_height_ * output_width_,
                     number_of_input_channels_, padded_offsets_weights_.data(), col_buffer_.data(),
                     top_expanded_data_.data(),
                     -INFINITY, 0, 0, num_regions_);
            interlace_cpu<T>(interlaced_top_buff_len_, top_expanded_data_.data(), top_expanded_diff_.data(),
            ...
            ggemm_readc_cpu
                    ...
                    (number_of_output_channels_, number_of_input_channels_,
                     ...
                     col_buffer_.data(), padded_offsets_weights_.data(), offsets_grad.data(), 0,
                     vec, num_regions_);
        }
        ...
        void LogVal(const TensorType &layer_input, TensorType & output_tensor, LookupType &lt) {
